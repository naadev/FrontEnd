...
function getdata() {
    ...
    var userLogon = sessionStorage.getItem("UserLoginResdec");
    if (userLogon.length > 0) {
        var urlApi = `http://127.0.0.1:5000/resdec/list_last_items_used/?username=${userLogon}&var_environment_id=1&number_items=10`;
...
function exit() {
    ...
    var url = `http://127.0.0.1:5000/resdec/logout/`;
    $.ajax({
        url: url,
